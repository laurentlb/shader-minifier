            ...
            | Decl (_, declElts) ->
                for def in declElts do
                    ...
                    | Some init ->
                        ...
                        let isConst = varsInStmt (Expr init) |> Seq.forall isEffectivelyConst
...
module private FunctionInlining =
    ...
    let tryMarkFunctionToInline funcInfo callSites =
        if not funcInfo.funcType.fName.DoNotInline && verifyArgsUses funcInfo.func callSites then
            ...
            debug $"{funcInfo.funcType.fName.Loc}: inlining function '{funcInfo.funcType}' into {callSites.Length} call sites"
